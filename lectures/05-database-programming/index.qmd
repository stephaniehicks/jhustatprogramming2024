...
```{r}
results <- dbGetQuery(mydb, "SELECT * FROM songs WHERE streams_millions > 600;")
print(results)
```
...
```{r}
results <- dbGetQuery(mydb, "SELECT title, release_year FROM songs WHERE release_year > 2010;")
print(results)
```
...
```{r}
results <- dbGetQuery(mydb, "SELECT album_id, SUM(streams_millions) AS total_streams 
                             FROM songs 
                             GROUP BY album_id;")
print(results)
```
...
- Join the songs and albums tables using SQL to get the album title for each song. Select the song title, album title, and release year.

::: {.callout-tip collapse="true"}
### Solution

```{r}
results <- dbGetQuery(mydb, "SELECT s.title AS song_title, a.album_title, s.release_year 
                             FROM songs s 
...
- Use SQL to find the average number of streams per album. Display the album title and the average streams.

::: {.callout-tip collapse="true"}
### Solution

```{r}
results <- dbGetQuery(mydb, "SELECT a.album_title, AVG(s.streams_millions) AS avg_streams 
                             FROM songs s 
                             JOIN albums a ON s.album_id = a.album_id 
