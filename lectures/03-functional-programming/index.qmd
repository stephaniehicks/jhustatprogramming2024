...
#### Part 4: Create nested lists and summaries

- First, use `split()` to divide mtcars into subsets based on the `cyl` column.

- Use `map()` to apply a summary function (e.g., mean, median) to each subset for columns like `mpg`, `hp`, and `wt` (weight).

- Then, take each subset from above, and for each one, use `map()` to calculate the mean and standard deviation of each column. **Organize the output into a nested list**, where each main element corresponds to a cylinder group, and each group contains the summary statistics for each column.
...
#### Part 5: Row-wise transformation and aggregation
...
#### Part 6: Visualizations and transformations
...
- How does `purrr` make it easier to perform repeated transformations compared to base R?
- What are some use cases where conditional transformations like `map_if()` could be helpful in real-world data?
